...
@section('main-content')
    <div class="row align-items-center mb-2">
        <div class="col">
          <div class="col">
              <h2 class="h3 page-title">View Teams</h2>
          </div>
          <div class="col-4">
              @foreach($users as $user)
                  @if($user->id == $team->iduser)
                      <p>On this page you can view the team {{ $team->name }} by {{ $user->name }}.</p>
                  @endif
              @endforeach
          </div>
        </div>
              ...
              <div class="col-12 text-left pl-4">
                <div class="align-items-center">
                  <div class="col-lg-12 col-md-12 col-sm-12">
                    @foreach($users as $user)
                      @if($user->id == $team->iduser)
                        <p class="h4">{{ $team->name }} by {{ $user->name }}</p>
                      @endif
                    @endforeach
                    ...
                    <div class="my-3">
                        <a href="{{ route('viewteams.index') }}">
                            <i class="fe fe-corner-down-left fe-16"></i> Return to View Teams
                        </a>
                    </div>
                  </div>
                </div>
